...
# import tf.contrib.graph_editor as ge
...
def gradients(ys, xs, grad_ys=None, checkpoints='collection', **kwargs):
        ...
        if checkpoints == 'collection':
            ...
            for ts in [ts_filtered, ts_all]:
                ...
                for t in ts:
                    ...
                    if not set(b_inp).intersection(f_inp) and len(b_inp) + len(f_inp) >= len(
                            ts_all):
    ...
    ge.reroute_ts(checkpoints_disconnected.values(), checkpoints_disconnected.keys(),
    ...
    for ts in checkpoints_sorted_lists[::-1]:
        ...
        ops_to_copy = fast_backward_ops(within_ops=fwd_ops, seed_ops=[r.op for r in ts],
        ...
        def _unsparsify(x):
            if not isinstance(x, tf.IndexedSlices):
                return x
            assert x.dense_shape is not None, "memory_saving_gradients encountered sparse " \
                                              "gradients of unknown shape"
            indices = x.indices
